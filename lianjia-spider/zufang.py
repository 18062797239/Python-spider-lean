...
def collect_area_zufang(city_name, area_name, fmt="txt"):
    """
    ...
    csv_file = today_path + "/{0}.txt".format(area_name)
...
if __name__ == "__main__":
    ...
    print(' 城市  City: {0}'.format(city))
    print(' 区域  Districts: {0}'.format(districts))

    # 获得每个区的板块, area: 板块
    areas = list()
    for district in districts:
        areas_of_district,zh_areas = get_areas(city, district,zh=1)
        print('{0}: 商圈  Area list:  {1}'.format(district, areas_of_district))
    ...
    print(" 区域  Area:", areas)
    print(" 区域&商圈  District and areas:", area_dict)

    # 准备线程池用到的参数
    nones = [None for i in range(len(areas))]       #[None, None, None, None, None, None]
    city_list = [city for i in range(len(areas))]   #['hf', 'hf', 'hf', 'hf', 'hf', 'hf']
